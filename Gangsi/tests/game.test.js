...
assert.strictEqual(room.phase, "adventurer_prepare");
...
assert(["classic", "test-map", "2"].includes(view.room.selectedMap.id));
assert.strictEqual(view.room.selectedMap.treasures.length, 23);
assert.deepStrictEqual(view.room.maps.map((map) => map.id), ["classic", "test-map", "2"]);
...

{
  ...
  assert(applyRoomAction(huntLobby, huntHuman, "chooseProfession", { profession: "engineer" }).includes("找不到"));
  assert.strictEqual(applyRoomAction(huntLobby, huntHuman, "chooseProfession", { profession: "archaeologist" }), null);
