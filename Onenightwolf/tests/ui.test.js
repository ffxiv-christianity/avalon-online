...
assert(!page.includes('<option value="120">'), "2-minute discussion is not a visible UI option");
assert(!page.includes('<option value="900">'), "15-minute discussion is not a visible UI option");
assert(!script.includes('aria-label="你的私人夜間結果"'), "dawn must not add a private-night-result region");
assert(!script.includes("const privateNightInfo = snapshot.you.privateInfo.length"), "discussion UI must not read hidden privateInfo");
