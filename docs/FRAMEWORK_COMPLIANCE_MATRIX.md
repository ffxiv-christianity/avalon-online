...
| Gangsi | 已套用 | 已套用 | 已套用 | 已套用 | 已套用 | 已套用，隱藏冒險者位置與未揭露任務 | 已測所有階段出口、阻擋、抓捕與雙方勝利 |

補充：LoveLetter 的無合法目標覆蓋必須逐卡驗證衛兵、神父、男爵與國王；隨機完整流程不能取代刻意建立「所有其他玩家都受保護」的契約測試。

補充：Gangsi 的完整隨機流程以資料驅動的等價覆蓋取代不可控的長時間漫遊；測試必須逐一涵蓋數字骰、箭頭骰、鎖骰／解鎖插入回合、正常提燈怪回合、被棋子阻擋、寶藏揭露、抓捕、地牢、雙方勝利與不同 viewer 的位置隔離。
