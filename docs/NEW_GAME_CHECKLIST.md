...
- [ ] 座位編號使用 `SharedRoomUI.seatNumber()`，並保留 `template-seat-number`。
...
- [ ] Shared room shell 的 Visual QA 必須覆蓋四款遊戲：Avalon、Onenightwolf、CriminalDance、LoveLetter；新型主遊戲 template marker 檢查只套用在使用該 template 的遊戲。
